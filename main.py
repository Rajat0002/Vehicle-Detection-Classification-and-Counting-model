...
kernalOp = np.ones((3,3),np.uint8)        #dilation
kernalOp2 = np.ones((5,5),np.uint8)       #dilation
kernalCl = np.ones((11,11),np.uint8)      #erosion 
...
with open('traffic_measurement.csv', 'w') as f:
    writer = csv.writer(f)
    csv_line = \
        'Vehicle Type, Vehicle Movement Direction,'
...
#------------------------------------------------------BINARIZATION----------------------------------------------------------------------------
    if ret==True:
        ...
        for cnt in contours0:
            ...
            if area>300:
                m=cv2.moments(cnt)  #reading frame of each rectangle
        ...
        for i in cars:
            ...
            if line_down+20<= i.getY() <= line_up-20:
               ...
               if a >= 100:
                     ...
                     cv2.putText(frame, "Big Car", (i.getX(), i.getY()), font, 1, (0,255,255), 2, cv2.LINE_AA)
               else:
                     cv2.putText(frame, "Car", (i.getX(), i.getY()), font, 1, (0,0,255), 2, cv2.LINE_AA)


        str_up='RIGHT_VEHICLE: '+str(cnt_up)
        str_down='LEFT_VEHICLE: '+str(cnt_down)
